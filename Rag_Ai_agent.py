...
from MULTIMODEL_TEXT import get_multimodal_data
...
def fuzzy_match_plates(original, detected):
    ...
    return similarity_ratio > 60  # Threshold can be adjusted
...
class AIAgent(autogen.AssistantAgent):
    ...
    def get_multimodal_input(self, input_type, user_input, user_query=None):
        """Handles multimodal input (text/image) and normalizes data."""
        try:
            if input_type == "text":
                text_data = get_multimodal_data(query=user_input)
                if not text_data:  # Ensure we got valid data
                    print("No valid data extracted from text input")
                    return None
                normalized = self._normalize_data(text_data)
                if not normalized:  # Ensure normalization worked
                    print("Failed to normalize text data")
                    return None
                return normalized
                ...
                if not os.path.exists(user_input):
                    ...
                    return None
                if not user_query:
                    print("No user query provided for image processing")
                    return None
                image_data = process_image(user_input, user_query)
                if not image_data:  # Ensure we got valid data
                    print("No valid data extracted from image")
                    return None
                normalized = self._normalize_data(image_data)
                if not normalized:  # Ensure normalization worked
                    print("Failed to normalize image data")
                    return None
                return normalized
        except Exception as e:
            ...
            if license_plate:
                for record in camera_records:
                    ...
                    if record_plate and fuzzy_match_plates(license_plate, record_plate):
                        print(f"License plate fuzzy match in camera {camera_id}")
                        return True
                ...
                if len(license_plate) >= 4:
                    ...
                    for record in camera_records:
                        ...
                        if len(record_plate) >=4 and record_plate.endswith(last_four):
                            print(f"License plate partial match in camera {camera_id}")
                            return True
